...
/// Returns the appropriate API key for the given service from the property list.
func apiKey(for service: APIProvider) -> String {
    guard let path = Bundle.main.path(forResource: apiKeysFileName, ofType: FileType.plist.rawValue) else { fatalError("An \(apiKeysFileName).plist file is missing from the project! Create a plist with the neccessary API services keys.") }
